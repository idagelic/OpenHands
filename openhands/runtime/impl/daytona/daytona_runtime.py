...
class DaytonaRuntime(ActionExecutionClient):
    ...
    def _get_sandbox(self) -> Sandbox | None:
        try:
            sandbox = self.daytona.get_sandbox(self.sandbox_id)
            self.log(
    ...
    def _create_sandbox(self) -> Sandbox:
        sandbox_params = CreateSandboxParams(
            ...
            resources=SandboxResources(cpu=3, memory=3, disk=5),
        )
    ...
    def _construct_api_url(self, port: int) -> str:
        assert self.sandbox is not None, 'Workspace is not initialized'
